...
let compoCache = (() => {
	...
	let buildVersion = '1';
	let cacheName = `appcache-gridmemo-NjQ1NTIxMDQ-build${buildVersion}`;
	...
	function cacheAssets(opt) {
		...
		fetch(manifestCachePath)
			...
			.then(async (json) => {
				...
				let cacheNames = await caches.keys();
				let hasLatestCache = cacheNames.find(
					(cname) => cname == newCacheVersionKey
				);
				...
				caches
					...
					.then(function () {
						...
						if (errorURLs.length > 0) {
							setInfo(
								'Failed to cache the following URLs. Offline access may work partially.'
							);
	...
	function loadData() {
		data = JSON.parse(
			localStorage.getItem(local.componentStorageKey) || local.defaultDataJSON
		);
	}
	...
	function GetSlotEl(itemEl) {
		let slotData = Array.from(itemEl.querySelectorAll('[data-slot]')).map(
			(x) => {
				return {
					key: x.dataset.slot,
					el: x,
	...
	return {
		...
		async Clear() {
			let isConfirm = window.confirm(
				'This will disable offline access. Continue?'
			);
			if (!isConfirm) return;
